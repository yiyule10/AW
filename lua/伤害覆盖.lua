...
if not imagepack_lib_installed then
    http.Get(
        "https://aimware28.coding.net/p/coding-code-guide/d/aimware/git/raw/master/imagepack_icons.lua?download=false",
...
local function waiting_download()
    if not pcall(gui.GetValue, "rbot.accuracy.custom.hicdmg.ind") then
        if network_error == "error" then
            ...
            draw.Text(0, 0, GetScriptName() .. "  链接不到网络")
        elseif not pcall(require, "imagepack_icons") then
            draw.Color(255, 255, 255, 255)
            draw.Text(0, 0, GetScriptName() .. "  正在下载图标库")
        else
            draw.Color(34, 170, 74, 255)
            draw.Text(0, 0, GetScriptName() .. "  下载成功请重新加载")
        end
    end
    return
end
...
local custom_hd_misc = gui.Groupbox(reference2, "自定义命中率&最低伤害", 329, 440, 296, 0)
...
local function for_gui()
    ...
    for i = 1, 11 do
        --- Hit chance
        ui.hitchance.enable[ui.wp_var[i]] = checkbox(custom_hd_misc, "custom." .. ui.wp_var[i] .. ".hitchance", "命中率", 0)
        ui.hitchance.ove_key[ui.wp_var[i]] = checkbox(custom_hd_misc, "custom." .. ui.wp_var[i] .. ".hitchance.key", "覆盖命中率按键", 0)
        ui.hitchance.default[ui.wp_var[i]] = slider(reference, "custom." .. ui.wp_var[i] .. ".hitchance.default", "默认命中率", 50, 0, 100)
        ui.hitchance.override[ui.wp_var[i]] =
            slider(reference, "custom." .. ui.wp_var[i] .. ".hitchance.override", "覆盖命中率", 50, 0, 100)
        ui.hitchance.inair[ui.wp_var[i]] = slider(reference, "custom." .. ui.wp_var[i] .. ".hitchance.inair", "空中命中率", 50, 0, 100)
        --- damage
        ui.mindmg.enable[ui.wp_var[i]] = checkbox(custom_hd_misc, "custom." .. ui.wp_var[i] .. ".mindmg", "最低伤害", 0)
        ui.mindmg.ove_key[ui.wp_var[i]] = checkbox(custom_hd_misc, "custom." .. ui.wp_var[i] .. ".hmindmg.key", "最低伤害覆盖按键", 0)
        ui.mindmg.visible[ui.wp_var[i]] = slider(reference, "custom." .. ui.wp_var[i] .. ".mindmg.visible", "可见时最低伤害", 50, 1, 130)
        ui.mindmg.autowall[ui.wp_var[i]] = slider(reference, "custom." .. ui.wp_var[i] .. ".mindmg.autowall", "穿墙最低伤害", 50, 1, 130)
        ui.mindmg.override[ui.wp_var[i]] = slider(reference, "custom." .. ui.wp_var[i] .. ".mindmg.override", "最低伤害覆盖", 50, 1, 130)
        ui.mindmg.inair[ui.wp_var[i]] = slider(reference, "custom." .. ui.wp_var[i] .. ".mindmg.inair", "空中最低伤害", 50, 1, 130)
    end
end
for_gui()
local x, y = draw.GetScreenSize()
local custom_hd_ind = gui.Checkbox(custom_hd_misc, "custom.hicdmg.ind", "指示器", 1)
